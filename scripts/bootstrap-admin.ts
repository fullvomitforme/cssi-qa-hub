...
if (!SERVICE_KEY)
  exitWithError("SUPABASE_SECRET_KEY or SUPABASE_SERVICE_ROLE_KEY is required")

// Validate email format
if (!EMAIL.includes("@"))
  exitWithError("BOOTSTRAP_ADMIN_EMAIL must be a valid email")

// Validate password strength
if (PASSWORD.length < 8)
  exitWithError("BOOTSTRAP_ADMIN_PASSWORD must be at least 8 characters")
...
async function bootstrap() {
  ...
  if (existingUser) {
    ...
    if (profile) {
      console.log(
        `Profile already exists: role=${profile.role}, status=${profile.status}`
      )
      console.log("")
