...
export async function getRunExecutionWorkspace(
  ...
  const { data, error } = await supabase
    .from("test_runs")
    .select(
      "id,name,status,build,started_at,completed_at,applications!inner(name,slug),environments!inner(name),releases!inner(version),test_run_assignments(profile_id,assigned_by,profiles!test_run_assignments_profile_id_fkey(full_name,role)),test_executions(id,source_scenario_id,scenario_title,scenario_description,scenario_preconditions,scenario_expected_result,scenario_priority,scenario_type,status,actual_result,failure_reason,severity,bug_reference,tested_at,tested_profile:profiles!test_executions_tested_by_fkey(full_name),source_scenario:test_scenarios!test_executions_source_scenario_id_fkey(modules!inner(name)),test_execution_steps(id,source_step_id,position,instruction,expected_result,status,actual_result),test_execution_attempts(id,attempt_number,status,build,actual_result,failure_reason,severity,bug_reference,executed_at),attachments(id,storage_path,filename,mime_type,size_bytes,uploaded_at,uploaded_profile:profiles!attachments_uploaded_by_fkey(full_name)),feedback(id,feedback_type,description,created_at,created_by_profile:profiles!feedback_created_by_fkey(full_name)))"
    )
    .eq("id", runId)
    .single()
